...
public class SockServer {
  ...
  static List<String> storyboard = new ArrayList<>();
  ...
  static JSONObject storyboard(JSONObject req) {
    ...
    if (req.getBoolean("view")) {
      res.put("ok", true);
      res.put("storyboard", new JSONArray(storyboard));
      res.put("users", new JSONArray(users));
    } else {
      if (!req.has("name") || !req.has("story")) {
        res.put("ok", false);
        res.put("message", "Error, missing value in request.");
      } else {
        String name = req.getString("name");
        String story = req.getString("story");
        synchronized (storyboard) {
          storyboard.add(story);
          users.add(name);
        }
        res.put("ok", true);
      }
...
static JSONObject charCount(JSONObject req) {
  System.out.println("Char count Request: " + req.toString());
  ...
  try {
    ...
    String count = req.getString("count");
    char find = req.getString("find").charAt(0);
    int cnt = 0;

    if (findchar) {
      for (int i = 0; i < count.length(); i++) {
        if (count.charAt(i) == find) {
          cnt++;
        }
      }
    } else {
      // loop through the string to count all chars
      cnt = 0;
      for (int i = 0; i < count.length(); i++) {
        if (count.charAt(i) != ' ') {
          cnt++;
    ...
    res.put("ok", false);
    res.put("message", "Error, please try again.");
  }
  return res;
}
